...
Let $\bar{y}_{s,t} = (\bmd_0, z_1, \bmd_1, \ldots, z_n, \bmd_n)$ with $n = s + t$.
...
\end{equation}
it is easy to verify that $P^{(0)}_s$ depends only on depths $z_{s'}$ and directions $\bmd_{s'}$ with $s' \leq s$, which are all from the subpath $\bar{x}_i$.
